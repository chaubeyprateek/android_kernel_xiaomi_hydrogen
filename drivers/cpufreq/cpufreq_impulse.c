...
struct cpufreq_impulse_tunables {
	int usage_count;
	/* Hi speed to bump to from lo speed when load burst (default 
max) */
...
#define DEFAULT_GO_HISPEED_LOAD 99
	...
	int ntarget_loads;
	/*
	 * The minimum amount of time to spend at a frequency before we 
can ramp
	 * down.
	 */
...
#endif
	/*
	 * Wait this long before raising speed above hispeed, by default 
a
	...
	bool boosted;
	/*
	 * Max additional time to wait in idle, beyond timer_rate, at 
speeds
	 * above minimum before wakeup to reduce speed, or -1 if 
unnecessary.
	 */
#define DEFAULT_TIMER_SLACK (4 * DEFAULT_TIMER_RATE)
	...
	/*
	 * Stay at max freq for at least max_freq_hysteresis before 
dropping
	 * frequency.
	 */
...
/* The caller shall take enable_sem write semaphore to avoid any timer 
race.
 * The policy_timer and policy_slack_timer must be deactivated when 
calling
 * this function.
 */
	...
	struct cpufreq_impulse_tunables *tunables, int cpu)
{
	struct cpufreq_impulse_policyinfo *ppol = per_cpu(polinfo, cpu);
	struct cpufreq_impulse_cpuinfo *pcpu;
	u64 expires = round_to_nw_start(ppol->last_evaluated_jiffy, 
tunables);
	unsigned long flags;
	int i;
	...
	unsigned int freq)
{
	...
	for (i = 0; i < tunables->nabove_hispeed_delay - 1 &&
			freq >= tunables->above_hispeed_delay[i+1]; i += 
2)
		;

	ret = tunables->above_hispeed_delay[i];
	spin_unlock_irqrestore(&tunables->above_hispeed_delay_lock, 
flags);
	return ret;
}
 ...
 * choose_freq() will find the minimum frequency that does not exceed 
its
		...
		unsigned int loadadjfreq)
{
	...
	do {
		prevfreq = freq;
		tl = freq_to_targetload(pcpu->policy->governor_data, 
freq);

		/*
		 * Find the lowest frequency where the computed load is 
less
		 * than or equal to the target load.
		 */

		if (cpufreq_frequency_table_target(
			    pcpu->policy, pcpu->freq_table, loadadjfreq 
/ tl,
		...
		if (freq > prevfreq) {
			...
			if (freq >= freqmax) {
				/*
				 * Find the highest frequency that is 
less
				 * than freqmax.
				 */
				if (cpufreq_frequency_table_target(
...
pcpu->freq_table[index].frequency;
				...
				if (freq == freqmin) {
					/*
					 * The first frequency below 
freqmax
					 * has already been found to be 
too
					 * low.  freqmax is the lowest 
speed
			...
			if (freq <= freqmin) {
				/*
				 * Find the lowest frequency that is 
higher
				 * than freqmin.
				 */
				if (cpufreq_frequency_table_target(
...
pcpu->freq_table[index].frequency;
				...
				/*
				 * If freqmax is the first frequency 
above
				 * freqmin then we have already found 
that
...
static void cpufreq_impulse_timer(unsigned long data)
{
	...
	u64 cputime_speedadj;
	int cpu_load;
	struct cpufreq_impulse_policyinfo *ppol = per_cpu(polinfo, 
data);
	struct cpufreq_impulse_tunables *tunables =
		ppol->policy->governor_data;
	struct cpufreq_impulse_cpuinfo *pcpu;
	...
	for_each_cpu(i, ppol->policy->cpus) {
		pcpu = &per_cpu(cpuinfo, i);
		if (tunables->use_sched_load) {
			cputime_speedadj = (u64)ppol->cpu_busy_times[i - 
fcpu]
					* 
...
#endif
	this_hispeed_freq = max(tunables->hispeed_freq, 
ppol->policy->min);
...
tunables->hispeed_freq)
	...
	if (cpufreq_frequency_table_target(ppol->policy, 
ppol->freq_table,
	...
	/*
	 * Do not scale below floor_freq unless we have been at or above 
the
	 * floor frequency for the minimum sample time since last 
validated.
	 */
	if (new_freq < ppol->floor_freq) {
		if (now - ppol->floor_validate_time <
				tunables->min_sample_time) {
			spin_unlock_irqrestore(&ppol->target_freq_lock, 
flags);
			goto rearm;
		}
	}
	...
	/*
	 * Update the timestamp for checking whether speed has been held 
at
	 * or above the selected frequency for a minimum of 
min_sample_time,
	 * if not boosted to this_hispeed_freq.  If boosted to 
this_hispeed_freq then we
	 * allow the speed to drop as soon as the boostpulse duration 
expires
...
spin_unlock_irqrestore(&speedchange_cpumask_lock,
			...
			spin_lock_irqsave(&speedchange_cpumask_lock, 
flags);
		}
...
static int load_change_callback(struct notifier_block *nb, unsigned long 
val,
				void *data)
{
...
static unsigned int *get_tokenized_data(const char *buf, int 
*num_tokens)
{
	const char *cp;
	int i;
...
static ssize_t show_hispeed_freq(struct cpufreq_impulse_tunables 
*tunables,
		char *buf)
{
...
static ssize_t store_hispeed_freq(struct cpufreq_impulse_tunables 
*tunables,
		const char *buf, size_t count)
{
	int ret;
...
static ssize_t show_timer_rate(struct cpufreq_impulse_tunables 
*tunables,
		char *buf)
{
	return sprintf(buf, "%lu\n", tunables->timer_rate);
...
static ssize_t store_timer_rate(struct cpufreq_impulse_tunables 
*tunables,
		const char *buf, size_t count)
{
	int ret;
	...
	if (val != val_round)
		pr_warn("timer_rate not aligned to jiffy. Rounded up to 
%lu\n",
			val_round);
...
static ssize_t show_timer_slack(struct cpufreq_impulse_tunables 
*tunables,
		char *buf)
{
...
static ssize_t store_timer_slack(struct cpufreq_impulse_tunables 
*tunables,
		const char *buf, size_t count)
{
	int ret;
...
static ssize_t show_io_is_busy(struct cpufreq_impulse_tunables 
*tunables,
		char *buf)
{
	return sprintf(buf, "%u\n", tunables->io_is_busy);
...
static ssize_t store_io_is_busy(struct cpufreq_impulse_tunables 
*tunables,
		const char *buf, size_t count)
{
	int ret;
...
static ssize_t show_powersave_bias(struct cpufreq_impulse_tunables 
*tunables,
		char *buf)
{
...
static ssize_t store_powersave_bias(struct cpufreq_impulse_tunables 
*tunables,
		const char *buf, size_t count)
{
	int ret;
					...
					struct cpufreq_policy *policy)
{
	...
	busy = kcalloc(cpumask_weight(policy->related_cpus), 
sizeof(*busy),
		       GFP_KERNEL);
	if (!busy) {
		kfree(ppol);
...
sysfs_remove_group(get_governor_parent_kobj(policy),
	...
	case CPUFREQ_GOV_START:
		...
		if (cpu_online(policy->cpu))
			cpufreq_impulse_timer_start(tunables, 
policy->cpu);
	...
	case CPUFREQ_GOV_LIMITS:
		...
		if (ppol->governor_enabled) {
			spin_lock_irqsave(&ppol->target_freq_lock, 
flags);
...
static int __init cpufreq_impulse_init(void)
{
	...
	sched_setscheduler_nocheck(speedchange_task, SCHED_FIFO, 
&param);
